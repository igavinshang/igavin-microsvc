package org.igavin.microsvc.elasticsearch.service.base;
...
import org.igavin.microsvc.elasticsearch.config.ElasticsearchProperties;
...
public abstract class BaseElasticsearchService {
    ...
    protected boolean isExistIndex(String index){
        ...
        boolean exist = false;
        try {
            exist = client.indices().exists(indexRequest, COMMON_OPTIONS);
    ...
    protected SearchResponse search(String index) {
        ...
        searchSourceBuilder.query(
                QueryBuilders.boolQuery()
                        .must(
                                QueryBuilders.matchQuery("country","US")
                        )
                        .filter(
                                QueryBuilders.rangeQuery("age").gte(11).lte(30)
                        )
        );
